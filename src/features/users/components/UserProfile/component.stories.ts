...
import { getMockUserById, mockWaitlistUsers } from "@/mocks/users.mock";
...
export const PendingUser: Story = {
	args: {
		...
		user: pendingUser!,
		referredUsers: mockWaitlistUsers.filter(
			(u) => u.referredBy === pendingUser!.id,
		),
		rewards: [],
	},
};
...
export const DesktopSocial: Story = {
	args: {
		...
		referredUsers: mockWaitlistUsers.filter(
			(u) => u.referredBy === desktopSocialUser!.id,
		),
 ...
 */
const fitnessUser = mockWaitlistUsers.find(
	(u) => u.campaignId === "campaign-2",
);
 ...
 */
const aiWritingUser = mockWaitlistUsers.find(
	(u) => u.campaignId === "campaign-4",
);
...
export const AIWritingUser: Story = {
	args: {
		...
		user: aiWritingUser!,
		referredUsers: mockWaitlistUsers.filter(
			(u) => u.referredBy === aiWritingUser!.id,
		),
...
export const InternationalUser: Story = {
	args: {
		...
		referredUsers: mockWaitlistUsers.filter(
			(u) => u.referredBy === internationalUser!.id,
		),
		rewards: [],
	},
};
