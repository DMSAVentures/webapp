 ...
 */
export const EmailTemplateEditor = memo<EmailTemplateEditorProps>(function EmailTemplateEditor({
	campaignId,
	type,
	template,
	onClose,
	...
	const [htmlBody, setHtmlBody] = useState(template?.html_body || defaultTemplate.htmlBody);
	const [textBody, setTextBody] = useState(template?.text_body || defaultTemplate.textBody);
	...
	// Hooks
	const { createTemplate, loading: creating, error: createError } = useCreateEmailTemplate();
	const { updateTemplate, loading: updating, error: updateError } = useUpdateEmailTemplate();
	const {
		sendTestEmail,
		loading: sendingTest,
		error: testError,
	...
	const handleSave = useCallback(async () => {
		const templateName = type === "verification" ? "Verification Email" : "Welcome Email";
	...
	}, [template, campaignId, type, subject, htmlBody, textBody, createTemplate, updateTemplate, onSave, handleClose]);
	...
	}, [campaignId, template, testEmailRecipient, sendTestEmail, resetTestState]);

	// Insert variable at cursor
	const insertVariable = useCallback((variable: string) => {
		const textarea = document.getElementById("email-html-body") as HTMLTextAreaElement;
		if (textarea) {
					...
					<div className={styles.editorPane}>
						<div className={styles.editorContent}>
							...
							<div className={styles.variablesSection}>
								<label className={styles.variablesLabel}>Insert Variable</label>
								<div className={styles.variablesList}>
									{TEMPLATE_VARIABLES.filter(
										(v) => type === "verification" || v.name !== "verification_link",
									).map((variable) => (
										<button
											key={variable.name}
											type="button"
