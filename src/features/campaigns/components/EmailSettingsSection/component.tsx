...
import { useGetEmailTemplates, type EmailTemplate } from "@/hooks/useEmailTemplates";
...
const TemplateCard = memo<TemplateCardProps>(function TemplateCard({
	...
	const typeDescriptions = {
		...
		welcome: "Sent after verification or immediately if verification is disabled",
	};
	...
	return (
		<div className={styles.templateCard}>
			...
			<div className={styles.templateCardContent}>
				<p className={styles.templateCardDescription}>{typeDescriptions[type]}</p>
 ...
 */
export const EmailSettingsSection = memo<EmailSettingsSectionProps>(function EmailSettingsSection({
	campaignId,
	...
	const verificationTemplate = templates.find((t) => t.type === "verification") || null;
	const welcomeTemplate = templates.find((t) => t.type === "welcome") || null;

	// Navigate to email builder
	const handleCustomize = () => {
		if (campaignId) {
			navigate({ to: `/campaigns/$campaignId/email-builder`, params: { campaignId } });
		}
	};
				...
				{!showVerificationTemplate && !showWelcomeTemplate && (
					<div className={styles.noEmailsMessage}>
						...
						<p className={styles.noEmailsHint}>
							Enable email verification above or toggle "Send welcome email" to configure
							automatic emails.
						</p>
					</div>
				)}
