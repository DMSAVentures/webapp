...
import { type DragEvent, type HTMLAttributes, memo, useState } from "react";
...
export const FormCanvas = memo<FormCanvasProps>(function FormCanvas({
	...
	const handleDrop = (e: DragEvent) => {
		...
		// Only handle drops on the canvas background (not on field items)
		// Field items have their own drop handler
		if (fieldType && !draggedFieldId && e.target === e.currentTarget) {
			// Adding new field from palette to the end
	...
	const handleFieldDrop = (e: DragEvent, dropIndex: number) => {
		...
		if (draggedFieldId) {
			...
			if (draggedIndex !== -1 && draggedIndex !== dropIndex) {
				...
				// If dragging from higher index to lower index, insert at dropIndex
				// If dragging from lower index to higher index, we need dropIndex-1 because we already removed the item
				const targetIndex =
