 ...
 */
const getRoleBadgeVariant = (role: TeamMember['role']): 'gray' | 'blue' | 'purple' | 'green' => {
  switch (role) {
    case 'owner':
      return 'purple';
    case 'admin':
...
export const TeamMembers = memo(
  function TeamMembers({
    ...
    // Handle invite
    const handleInvite = async (data: { email: string; role: TeamMember['role'] }) => {
    ...
    // Handle role change
    const handleRoleChange = async (memberId: string, option: DropdownOptionInput) => {
          ...
          <>
            ...
            {activeMembers.length > 0 && (
              <div className={styles.section}>
                <h3 className={styles.sectionTitle}>Active Members ({activeMembers.length})</h3>
                  ...
                  {activeMembers.map((member) => (
                    <div key={member.id} className={styles.tableRow}>
                      ...
                      <div className={styles.tableCell}>
                        {member.role === 'owner' ? (
                          <Badge
                            text={member.role.charAt(0).toUpperCase() + member.role.slice(1)}
                          ...
                          <div className={styles.roleDropdown}>
                            <Dropdown
                              ...
                              size="x-small"
                              onChange={(option) => handleRoleChange(member.id, option)}
            ...
            {pendingInvitations.length > 0 && (
              <div className={styles.section}>
                <h3 className={styles.sectionTitle}>Pending Invitations ({pendingInvitations.length})</h3>
                <div className={styles.table}>
                  <div className={styles.tableHeader}>
                    <div className={styles.tableCell}>Member</div>
                    <div className={styles.tableCell}>Role</div>
                    ...
                    <div key={member.id} className={styles.tableRow}>
                      <div className={styles.tableCell}>
                        <div className={styles.memberInfo}>
                          ...
                          <div className={styles.memberDetails}>
                            <div className={styles.memberEmail}>{member.email}</div>
                            <div className={styles.pendingLabel}>Pending invitation</div>
                          </div>
                        </div>
                      </div>
                      <div className={styles.tableCell}>
