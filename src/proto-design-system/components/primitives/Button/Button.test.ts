...
test.describe("Button Component", () => {
  ...
  test.describe("Functionality", () => {
    test("renders with default props", async ({ page }) => {
      await page.goto(`${STORYBOOK_URL}/iframe.html?id=primitives-button--default`);
    ...
    test("displays loading state correctly", async ({ page }) => {
      await page.goto(`${STORYBOOK_URL}/iframe.html?id=primitives-button--loading`);
      const button = page.getByRole("button");
    ...
    test("disabled state prevents interaction", async ({ page }) => {
      await page.goto(`${STORYBOOK_URL}/iframe.html?id=primitives-button--disabled`);
      const button = page.getByRole("button");
    ...
    test.describe("Variants", () => {
      const variants = ["default", "primary", "secondary", "ghost", "outline", "destructive"];

      for (const variant of variants) {
        test(`renders ${variant} variant`, async ({ page }) => {
          await page.goto(`${STORYBOOK_URL}/iframe.html?id=primitives-button--${variant}`);
          const button = page.getByRole("button");
          await expect(button).toBeVisible();
        });
      }
    ...
    test.describe("Sizes", () => {
      ...
      for (const size of sizes) {
        test(`renders ${size} size`, async ({ page }) => {
          await page.goto(`${STORYBOOK_URL}/iframe.html?id=primitives-button--${size}`);
          const button = page.getByRole("button");
          await expect(button).toBeVisible();
        });
      }
    ...
    test("renders with left icon", async ({ page }) => {
      await page.goto(`${STORYBOOK_URL}/iframe.html?id=primitives-button--with-left-icon`);
      const button = page.getByRole("button");
      await expect(button).toBeVisible();
    ...
    test("renders with right icon", async ({ page }) => {
      await page.goto(`${STORYBOOK_URL}/iframe.html?id=primitives-button--with-right-icon`);
      const button = page.getByRole("button");
      await expect(button).toBeVisible();
    ...
    test("full width spans container", async ({ page }) => {
      await page.goto(`${STORYBOOK_URL}/iframe.html?id=primitives-button--full-width`);
      const button = page.getByRole("button");
      await expect(button).toBeVisible();
    ...
    test("icon-only button is square", async ({ page }) => {
      await page.goto(`${STORYBOOK_URL}/iframe.html?id=primitives-button--icon-only-default`);
      const button = page.getByRole("button");
      await expect(button).toBeVisible();

      const buttonBox = await button.boundingBox();
      expect(Math.abs((buttonBox?.width ?? 0) - (buttonBox?.height ?? 0))).toBeLessThan(2);
    });
  });
  ...
  test.describe("Accessibility", () => {
    ...
    test("has no accessibility violations (all variants)", async ({ page }) => {
      await page.goto(`${STORYBOOK_URL}/iframe.html?id=primitives-button--all-variants`);
    ...
    test("maintains sufficient color contrast", async ({ page }) => {
      await page.goto(`${STORYBOOK_URL}/iframe.html?id=primitives-button--primary`);
