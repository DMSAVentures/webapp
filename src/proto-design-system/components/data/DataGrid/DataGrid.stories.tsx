...
const columns: DataGridColumn<User>[] = [
    ...
    cell: (row) => (
      <span
        style={{
          ...
          fontWeight: 500,
          background:
            row.status === "active" ? "var(--color-success-100)" : "var(--color-base-100)",
          color: row.status === "active" ? "var(--color-success)" : "var(--color-muted)",
        }}
      >
        {row.status}
      </span>
  ...
  { id: "lastLogin", header: "Last Login", cell: (row) => row.lastLogin, align: "right" },
];
...
export const Default: Story = {
  render: () => (
    <DataGrid data={sampleData.slice(0, 5)} columns={columns} getRowId={(row) => row.id} />
  ),
};
    ...
    <DataGrid
      ...
      emptyState={
        <div style={{ padding: "2rem", textAlign: "center" }}>
          <p style={{ fontSize: "var(--font-size-lg)", fontWeight: 500, marginBottom: "0.5rem" }}>
            No users found
          </p>
      ...
      <div>
        ...
        >
          ...
          <span style={{ color: "var(--color-muted)", fontSize: "var(--font-size-sm)" }}>
            {selectedIds.size > 0 ? `${selectedIds.size} selected` : `${sampleData.length} total`}
          </span>
        </div>
        <DataGrid
          data={sampleData}
...
export const CustomColumns: Story = {
  render: () => {
    ...
    return (
      <DataGrid data={sampleData.slice(0, 5)} columns={customColumns} getRowId={(row) => row.id} />
    );
  },
};
