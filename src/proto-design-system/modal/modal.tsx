...
import { Button } from "@/proto-design-system/Button/button.tsx";
...
const ModalHeader: React.FC<ModalHeaderProps> = (props) => {
	...
	return (
		<div
			className={`${styles["modal__header"]} ${props.centeredHeader ? styles["modal__header--centered"] : ""}`}
		>
			{props.icon && (
				<i
					className={`${styles["modal__icon"]} ${props.description ? styles["modal__icon--medium"] : ""} ${styles[`modal__icon--${props.icon}`]} ${iconComponent}`}
				></i>
			)}
			...
			{!props.centeredHeader && props.dismissibleByCloseIcon && (
				<IconOnlyButton
					ariaLabel={"modal-icon"}
					variant={"secondary"}
					iconClass={"close-fill"}
		...
		>
			<div className={styles["modal__content"]}>
				<ModalHeader
					...
					title={props.title}
					onClose={closeModal}
					dismissibleByCloseIcon={props.dismissibleByCloseIcon ?? true}
					centeredHeader={props.centeredHeader}
				/>
				{props.children && (
					<div className={styles["modal__body"]}>{props.children}</div>
				)}
				<ModalFooter
					footerFullWithButtons={props.footerFullWithButtons}
					footerLeftChildren={props.footerLeftChildren}
