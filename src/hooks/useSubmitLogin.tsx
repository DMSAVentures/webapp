...
export const useSubmitLogin = (): SubmitLoginOperation => {
    ...
    const submitLogin = async (email: string, password: string): Promise<void> => {
        setLoading(true);
        setError(null);

        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL}/api/auth/login/email`, {
                method: "POST",
                headers: {
            ...
            if (error instanceof Error) {
                ...
                setError({ code: "unknown_error", message: "An error occurred during login" });
            }
        } finally {
            setLoading(false);
        }
